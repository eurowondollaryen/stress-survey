  ...
  try {
    for (let i = 0; i < keyStringList.length; ++i) {
      ...
      for (let j = 0; j < result.length; ++j) {
        maxScoreArr[result[j]["qstn_div"]] =
          commonFunctions.nullToZero(maxScoreArr[result[j]["qstn_div"]]) + 4;
        totalScoreArr[result[j]["qstn_div"]] =
          commonFunctions.nullToZero(totalScoreArr[result[j]["qstn_div"]]) +
          result[j]["qstn_ans"];
        questionCountArr[result[j]["qstn_div"]] =
          commonFunctions.nullToZero(questionCountArr[result[j]["qstn_div"]]) +
          1;
      }
      ...
      for (let j = 0; j < maxScoreArr.length; ++j) {
        ...
        resultArr[j] =
          ((totalScoreArr[j] - questionCountArr[j]) /
            (maxScoreArr[j] - questionCountArr[j])) *
          100;
      }

      //계산 결과를 저장
      for (let j = 0; j < resultArr.length; ++j) {
        calculation.saveResult([
          keyString[0],
          keyString[1],
          keyString[2],
          keyString[3],
...
const searchCalculationResult = async (req, res) => {
  const { USER_ID } = req.query;

  try {
    const result = await calculation.searchCalculationResult([USER_ID]);
    console.log(
      "[calculationController][searchCalculationResult] request success!"
    );
    res.json(result);
  } catch (err) {
    console.error(err.message);
...
exports.searchCalculationResult = searchCalculationResult;
